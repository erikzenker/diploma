...
%                                                                              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo{Search for more sources, but content should be enought}
...
processing units (CPUs) and with a crowing popularity also with one or
more accelerator devices.

Accelerator devices are some kind of coprocessor which are usually
able to execute operations in a massively parallel manner.
General-purpose computing on graphics processing units (GPGPU) enters
with its high speedups the world of compute clusters.  These graphics
processing units (GPUs) are mainly from AMD or
Nvidia. OpenCL\cite{ref:opencl} is the currently dominant open
general-purpose GPU computing language. The dominant proprietary
framework is Nvidia's CUDA\cite{ref:cuda}.

Also the many core archtitectures xeon phi\cite{ref:xeon_phi} from
intel tries to establish iself in the compute cluster marked. It
should be programmed by OpenMP and tries to accelerate mainly existing
OpenMP code without any further changes in the programm.

Usually, the accelerator devices need a CPUs to manage the memory
...
But accelerator devices have in the future the objective to run in a
standalone or native mode, where a host is not needed anymore.  The
Xeon Phi is allready able to run in native mode.

The first places in the TOP500 list of supercomputers are all
accelerated by accelerator devices, that shows the developement of
compute clusters to a heterogeneous system archtitecture.
...
Communication plays an important role in complex simulation
applications. It is not possible anymore that these complex simulation
applications fit on a single compute device, because they are memory
bounded. Thus the simulation need to be decomposed in subdomains and
distributed to more compute devices.  The Subdomains are usually not
independent from each other, they need to exchange data. There are
several methods to exchange data between the nodes.
...
Active messages \cite{ref:am} is a slight departute from the classical
...
which will be explained in section \ref{sec:mpi}.
...
single computer.  These distributed shared architectures make it
possible to use standard and simple parallization schemas like
Pthreads or OpenMP.  Communication between the nodes does not have to
be modeld explicitly, thus it is easy to use for non computer
scientiest. But with this kind of higher abstraction allways comes a
loose of control and therefore also efficiency.
...
by consecutive integers from 0 to groupsizie-1. Which the initial
group compromises the full set of processes with ranks in the range 0
to the number of processes. A typical MPI program run is composed from
the following steps:
...
NVIDIA GPU-accelerated nodes, whereby the host CPUs are only utilized
for memory transfers between CPU and GPU and for internode
communication via MPI (Section \ref{sec:mpi}).
...
subdomains. Each subdomain is mapped to its own GPU. Particles, fields
etc. can cross the border of the subdomain and therfore data between
subdomains has to be exchanged. The Data will be copied onto the host
memory and is then transfered to the host of the neighboring
subdomain. The Communication between the nodes of PIConGPU is handled by MPI.
After that the memory is copied to the GPU memory of the
neighboring subdomain host. The data transfer between CPU and GPU is
also called offload.



The during this thesis developed communication layer was leaded
by the needs of PIConGPU as an example application for
high performance computing on cluster systems.
...
neighboring cells. To decouple communication and computation, nonblocking variant
of this basic operations are used.
