...
transformation, the \cpp{MPIOperator} can be used by all MPI routines
...
\cpp{MPI\_Wait} on the request (Listing \ref{lst:mpi_event_wait}). 
...
already finished (Listing \ref:mpi_event_ready}).
...
\item The core components of the GVON are several mappings.
  The \cpp{vertexMap} is a mapping from a vertex of a graph to
...
\sitem{implements the interface claimed by the gvon design}
\sitem{Similar to Adapter methods, because ...}
\sitem{translation from vertex to host virtual address}
\sitem{translation from graph to context}
...
\cpp{T_Data} for a root vertex (Listing \ref{collective}). The data is
...
The occupied vertex does not need to be adjacent to hosted vertices
  ...
  \sitem{Even when this was just an simple example it shows remapping of vertices at runtime}
  \sitem{This can be used for load balancing and fault tolerance}
