...
number of data elements.  The method \cpp{data()} should return the
...
\cpp{std::vector} or \cpp{std::array} objects \cite{ref:vector,
  ref:array}. Listing \ref{lst:cal_async_send} shows the
implementation of the \cpp{asyncSend()} method of the CAL. It shows
...
claimed by the interface. Utilizing \cpp{sendData.data()} and
...
MPI provides built-in support for binary operations by two approaches. The
...
\item Distribute vertices
  ...
  focus of this work (Listing \ref{lst:gol_distribution}).
...
vector<Vertex> hostedVertices = Dist::roundrobin(context, graph);
  ...
  \noindent Other arbitrary distribution methods are also possible. Figure
  ...
  is an interesting case for fault tolerance and load balancing.
