...
(IPC). This might not be the most efficient implementation of GoL,
because a single process can compute considerably more cells by its
own.

Figure \ref{fig:gol_bundle} partitions the GoL graph by bundling
multiple vertices together and models a new graph. Bundles are
...
partitioned graph represents dependencies of bundled cells.
...
%% Requirements
%% Motivation stinkt !
The Domain of an application was modeled with the help of graphs in
the previous section \ref{sec:graph}. A graph serves as the basis
for communication or data exchange within the domain.

Thus edges of the graph can be interpreted as communication links
between peers


The graph does only provide
domain specific information. 

But the graph can be
used as the foundation for communication or data exchange within the
domain.

This section introduces a graph based overlay network creates an overlay network on top.

The NameService is the connection of the graph as the topology
description instance and the peers of the CAL as communication
interface.

It contains two mappings. Vertices of a graphs are mapped to
peers and graphs are mapped to contexts.




%% Process of announce
\subsection{Announce hosting peers}
The objective is to establish a communication overlay on top of the
network provided by the CAL. This overlay network is based on the
graphs introduced in section \ref{sec:graph}. 

This Graph can be constructed in parallel by all peers, loaded from a
distributed file system or could even be delivered by a master
peer. However, it is important that all peers that want to take part
on the communication on this overlay network need to know the same
graph.

The next phase is the distribution of the vertices of the graph to the
peers. This has the meaning that a peer is responsible for the
communication of these vertices. The peer will be called host of
the vertices.

There are a varity of methods to distribute the vertices of a graph
to the peers of a network. It could be done totally randomized, round
robin or even dictated by some master peer. The distribution behavior
is defined be the user of the library and might be object for
further optimization.

Every peer hosts zero, one or more vertices, so to speak it is
also possible that one peer host all vertices and communicates
finally always with itself.  

Thus the peers announce their hosted vertices to all other peers,
which will store this information. This is done by the collective
operation allGather.

%% Remapping of vertices
\subsection{Reannounce}
Because the communication hardware and topolgie is seperated,
it is also possible to change the mapping from vertices to 
peers at runtime. That can be necessary when load
of an application is getting inbalanced during runtime.
...
and then announced. The difference is, that the peers
...
\subsection{Communication on the modled domain}
