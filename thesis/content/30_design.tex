...
\begin{description}
  ...
  \item[Context] This gives MPI the information which processes are
    able to communicate with each other. Hint: Context and tag could
    be removed, thus some default tag and a global context would be
    choosen for the operations.

\end{description}

It is not planed that the CAL provides routing abilities. Peers have
to be connected directly by the network of the adapter. No routing
makes it also impossible to forward data between different adapters of
the CAL. Thus a specific CAL only provides a single adapter. But
several CALs can be used with different adapters to communcate on
several networks. In such a scenario the routing can be implemented on
top of the CAL by the library user. future work could implement an
routing underneath the CAL to reach also not directly connected peers
automatically.
...
algorithms based on algorithms with more than 2 participating
peers.These operations where introduced as collective
operations\ref{sec:collectives}.
